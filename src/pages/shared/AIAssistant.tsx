...
import { Bot, Send, AlertTriangle, TrendingUp, RefreshCw } from 'lucide-react';
...
export default function AIAssistant() {
  ...
  const runDiseaseDetection = async () => {
    ...
    if (!farmId) return;
    setDiseaseBusy(true); setDiseaseError(''); setDiseaseResult(null);
    try {
      const res = await aiService.detectDisease(farmId);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 'var(--sp-lg)', flexWrap: 'wrap' }}>
        {(['chat'] as Tab[]).concat(showDiseaseTab ? ['disease'] : []).concat(showCreditTab ? ['credit'] : []).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
            ...
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', marginTop: 60, color: 'var(--col-muted)' }}>
                ...
                <p style={{ margin: 0, fontSize: 14 }}>
                  Ask me anything about your {role === 'farmer' ? 'farm, flock health, credit, or training' :
        ...
        <div>
          <Card>
            ...
            <p style={{ fontSize: 14, color: 'var(--col-muted)', margin: '0 0 var(--sp-md)' }}>
              Analyse recent activity logs to detect early disease signals and health risks.
            </p>
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              {farmList.length === 0 && role === 'admin' && (
                <div className="form-field" style={{ flex: 1, minWidth: 200, margin: 0 }}>
                  <label>Farm ID</label>
                  <input type="text" placeholder="Enter farm UUID" value={selectedFarm} onChange={e => setSelectedFarm(e.target.value)} />
                </div>
              )}
              <Button onClick={runDiseaseDetection} disabled={diseaseBusy || (farmList.length === 0 && !selectedFarm)}>
                {diseaseBusy ? <><RefreshCw size={14} style={{ marginRight: 6, animation: 'spin 1s linear infinite' }} />Analysing…</> : '🦠 Run Analysis'}
              </Button>
            </div>
          ...
          {diseaseResult && (
            <Card style={{ marginTop: 'var(--sp-md)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 'var(--sp-md)' }}>
                <div style={{
                ...
                <span style={{ fontSize: 13, color: 'var(--col-muted)' }}>Risk score: <strong>{diseaseResult.risk_score}/100</strong></span>
